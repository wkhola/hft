...
class EtlWifiBlob(object):
    def __init__(self):
        ...
        self.container = self.conf.get("pro_blob", "container")
        self.block_blob_service = BlockBlobService(account_name=self.conf.get("pro_blob", "account_name"),
                                                   account_key=self.conf.get("pro_blob", "account_key"),
                                                   endpoint_suffix=self.conf.get("pro_blob", "endpoint_suffix"))
    ...
    def delete_blob(self, date):
        ...
        for blob in generator:
            # 获取指定日期的所有数据
            if date in str(blob.name) and 'zip' not in str(blob.name):
                self.block_blob_service.delete_blob(self.container, blob.name)

    def zip_write_blob(self, date, file_path='./'):
        """
        将本地文件压缩后上传
        :param date: 指定日期
        ...
        try:
            zip = zipfile.ZipFile(os.path.join(file_path, date) + ".zip", 'w', allowZip64=True)
...
class LocalFileEtl(object):
    ...
    def __fmt_msg(self, dfm):
        ...
        df = pd.concat(files) \
            # .sort_values(by=['shop_id', 'mac', 'time'])  # 合并blob文件
    ...
    def mer_daily_data(self, dfm):
                    ...
                    files.append(
                        pd.read_csv("{}/{}".format(self.result_dir, filename),  # header=0 这个参数在合并的时候去掉表头
                                    ...
                                    names=["shop_id", 'cus_mac', 'sen_mac', 'frame_big', 'frame_little', 'channel',
                                           'intensity', 'time']).drop_duplicates()
                        # .sort_values(by=['shop_id', 'cus_mac', 'time'])
                    )
        ...
        columns = ['shop_id', 'cus_mac', 'sen_mac', 'frame_big', 'frame_little', 'channel', 'intensity', 'time']
        # df = df.sort_values(by=['shop_id', 'cus_mac', 'time'])
        df.to_csv('./{}.csv'.format(dfm), chunksize=10000,
                  index=False, sep=',', encoding="utf_8", columns=columns)
        # 将合并后的本地初始文件删除
        for filename in os.listdir(self.result_dir):
            if dfm in str(filename):
                # print filename
                os.remove(os.path.join(self.result_dir, filename))

    # 移动生成好的csv文件
    def move_file(self, dfm):
        for filename in os.listdir(self.dir):
            if dfm in str(filename) and 'csv' in str(filename):
                shutil.move(os.path.join(self.dir, filename), os.path.join(self.daily_dir, filename))
            if dfm in str(filename) and 'zip' in str(filename):
...
if __name__ == '__main__':
    ...
    for i in range(2, 17):
        try:
            if i <= 9:
                yesterday = date + "-0{0}".format(i)
            elif 9 < i <= 19:
                yesterday = date + "-{0}".format(i)
            else:
                yesterday = date + "-{0}".format(i)
        except Exception, e:
            logger.error(traceback.format_exc(e))
        ...
        # etl_wifi_blob.delete_blob(yesterday)
        # logger.info('删除完毕')
        #
        # # 合并文件成小时文件
        # local_file_blob.mer_msg(yesterday)
        # logger.info("小时文件合并完毕")
        #
        # # 合并文件成一天一个大文件
        ...
        logger.info("上传blob完成")

        # 处理本地文件
        local_file_blob.move_file(yesterday)
        logger.info("本地文件处理完毕")
